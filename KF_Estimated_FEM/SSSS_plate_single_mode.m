...
sensor_global_dof = (sensorNode(1)-1)*dof_per_node + 1; % DOF 1 is 'w'
...
Px = 1e-7*eye(2*modes); % State covariance matrix
Qx = 1e-8 * eye(2*modes); % Process noise covariance
Pfd = 7e-13*eye(params);
Qfd = 2e-13*eye(params);
...
% for accn
figure('Name', 'Time History Response');
plot(time, sensor_accn_hist_true, 'k-', 'LineWidth', 1.5); hold on;
plot(time, a_dkf_hist, 'b--', 'LineWidth', 1.5);
plot(time, a_dkf_hist_state, 'm--', 'LineWidth', 1.5);
grid on;
title(['Vertical Accn at Sensor Node ' num2str(sensorNode(1))]);
xlabel('Time (s)');
ylabel('Accn (m/s2)');
xlim([0, T_total]);
