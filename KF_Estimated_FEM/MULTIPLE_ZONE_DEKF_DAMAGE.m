...
fd(1,:) = linspace(0,0.8,nt+1);
fd(3,:) = linspace(0,0.3,nt+1);
fd(4,:) = linspace(0,0.2,nt+1);
...
%Px = blkdiag(1e-12,1e-12,  1e-12,1e-14, 1e-12,1e-12,  1e-12,1e-14, 1e-12,1e-12,  1e-12,1e-14,  1e-12,1e-12,  1e-12,1e-14, 1e-12,1e-12,  1e-12,1e-14, 1e-12,1e-12,  1e-12,1e-14); % State covariance matrix
Px = 1e1*(eye(2*modes));
%Qx = blkdiag(1e-12,1e-12,  1e-12,1e-14, 1e-12,1e-12,  1e-12,1e-14, 1e-12,1e-12,  1e-12,1e-14,  1e-12,1e-12,  1e-12,1e-14, 1e-12,1e-12,  1e-12,1e-14, 1e-12,1e-12,  1e-12,1e-14); % Process noise covariance
Qx = 1e-2*(eye(2*modes));
%Pfd = 7e-12*eye(params); Qfd = 8e-12*eye(params);
Pfd = 1e-3*blkdiag(0.0010,0.0025 ,  0.1105,0.0071, 0.0216,0.0822 );
Qfd = 1e-3*blkdiag(0.0010,0.0025 ,  0.1105,0.0071, 0.0216,0.0822 );

R_u = 1e-1 * eye(params);
R_v = 1e-1 * eye(params);
%R_u = blkdiag(8e-6,8e-6,  8e-6,8e-6, 8e-6,3e-6 );
%R_v = blkdiag(8e-6,8e-6,  8e-6,8e-6, 8e-6,3e-6 );
Rx = blkdiag(R_u, R_v);           % measurement covariance for u and v (2*Ns x 2*Ns)
% Rfd = 1e-5 * eye(params); 
Rfd = blkdiag(1e2,1e2,  1e2,1e2, 1e2,1e2 );
...
% % Initialize
  q  = zeros(modes,nt+1);
  qd = zeros(modes,nt+1);
  qdd = zeros(modes,nt+1);
  F = F_modal_red*sin(omegan(1)*(0:nt+1)*dt);
  
 Minv = inv(M_modal_red);
 
% --- Newmark-Beta Parameters ---
gamma = 0.5;
beta = 0.25;

% --- Pre-calculate Integration Constants ---
a1 = 1/(beta*dt^2);
a2 = 1/(beta*dt);
a3 = (1/(2*beta)) - 1;
a4 = gamma/(beta*dt);
a5 = (gamma/beta) - 1;
a6 = (dt/2) * ((gamma/beta) - 2);

% --- Effective Stiffness Matrix (Pre-calculate outside the loop) ---
% Note: If you have damping C, add + a4*C_modal_red here
K_eff = K_modal_red + a1*M_modal_red; 

% --- Initial Acceleration (for n=1) ---
qdd(:,1) = M_modal_red \ (F(:,1) - K_modal_red*q(:,1));

for n = 2:nt+1
    % 1. Calculate Effective Load for the current step
    % This accounts for the mass and stiffness properties from the previous step
    R_eff = F(:,n) + M_modal_red*(a1*q(:,n-1) + a2*qd(:,n-1) + a3*qdd(:,n-1));
    
    % 2. Solve for Displacement at step n
    q(:,n) = K_eff \ R_eff;
    
    % 3. Update Acceleration and Velocity for step n
    % These formulas ensure the system remains energetically stable
    qdd(:,n) = a1*(q(:,n) - q(:,n-1)) - a2*qd(:,n-1) - a3*qdd(:,n-1);
    qd(:,n)  = qd(:,n-1) + a6*qdd(:,n-1) + (gamma*dt)*qdd(:,n);
end

Observability_fd = zeros(params,1);
for z=1:Nz
    for i=1:T_total
        %tmp = (M_modal_red + dt*0.5*C_modal_red + dt^2*0.25*K_dkf_modal)\ (Kz_modal_red{z} * (x_state_hist(1:modes,i) + dt*x_state_hist(modes+1:end,i) + dt^2*0.25*a_dkf_prev));    % modes x 1
        tmp = (M_modal_red + dt*0.5*C_modal_red + dt^2*0.25*K_dkf_modal)\ (Kz_modal_red{z} * (q(:,i) + dt*qd(:,i) + dt^2*0.25*qdd(:,i)));    % modes x 1
        
        D_fd(:, i) = Phi_sen * tmp;   
    end
    Observability_fd(z) = mean(vecnorm(D_fd,2,1));
end
% normalize to [0,1]
Observability_fd = Observability_fd / max(Observability_fd);   


epsi = 1e-3;  
p0 = 1e-2;               % base variance
P2_0 = diag(p0 ./ (Observability_fd + epsi));
q0 = 1e-5;               % base process noise
Q2 = diag(q0 ./ (Observability_fd + epsi));

P2_0 = min(P2_0, 1e2*eye(params))

Q2   = min(Q2,   1e-1*eye(params))

figure
bar(Observability_fd)
xlabel('Damage zone')
