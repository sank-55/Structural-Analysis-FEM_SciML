...
for i = 2:N
    ...
    zst_measured(:,i) = [Phi_mat *( x_new + x_new*0.02*randn); Phi_mat *( v_new + v_new*0.02*randn)];
    zacc_measured(:,i) = Phi_mat *( a_new + a_new*0.03*randn);
...
optimVars_ekf = [
    optimizableVariable('p_state_exp', [-22, -14], 'Type', 'real')
    optimizableVariable('p_param_exp', [-20, -8], 'Type', 'real')
    optimizableVariable('q_state_exp', [-16, -10], 'Type', 'real')
    optimizableVariable('q_param_exp', [-12, -6], 'Type', 'real')
...
optimVars_dkf2 = [
    optimizableVariable('p_state_dkf2', [-22, -18], 'Type', 'real')
    optimizableVariable('p_param_dkf2', [-30, -26], 'Type', 'real')
    optimizableVariable('q_state_dkf2', [8, 12], 'Type', 'real')
    optimizableVariable('q_param_dkf2', [-12, -8], 'Type', 'real')
    optimizableVariable('r_state_dkf2', [1, 5], 'Type', 'real')
    optimizableVariable('r_param_dkf2', [-5,-1], 'Type', 'real')
    optimizableVariable('rval', [0, 1], 'Type', 'real')
...
optimVars_dkf1 = [
    optimizableVariable('p_state_dkf1', [-28, -22], 'Type', 'real')
    optimizableVariable('p_param_dkf1', [-28, -22], 'Type', 'real')
    optimizableVariable('q_state_dkf1', [12, 18], 'Type', 'real')
    optimizableVariable('q_param_dkf1', [-12, -8], 'Type', 'real')
    optimizableVariable('r_state_dkf1', [-2, 2], 'Type', 'real')
    optimizableVariable('r_param_dkf1', [-6, 1], 'Type', 'real')
...
results_ekf = bayesopt(ObjFcn_ekf, optimVars_ekf, 'MaxObjectiveEvaluations', 50, 'PlotFcn', {});

fprintf('\nStarting Bayesian Optimization dkf1 ...\n');
results_dkf1 = bayesopt(ObjFcn_dkf1, optimVars_dkf1, 'MaxObjectiveEvaluations', 50, 'PlotFcn', {});

fprintf('\nStarting Bayesian Optimization dkf2 ...\n');
results_dkf2 = bayesopt(ObjFcn_dkf2, optimVars_dkf2, 'MaxObjectiveEvaluations', 50, 'PlotFcn', {});
...
%%
figure(15);
sgtitle('Damage Estimation with 10 Sensor(Most Obsv point)');
for i = 1:Ndam
    % --- 1. Calculate RMSE for each filter ---
    rmse_dkf1 = sqrt(mean((damf_true(i,:) - fd_dkf1_optimized(i,:)).^2));
    rmse_dkf2 = sqrt(mean((damf_true(i,:) - fd_dkf2_optimized(i,:)).^2));
    rmse_ekf  = sqrt(mean((damf_true(i,:) - fd_est(i,:)).^2));
    
    % --- 2. Plotting ---
    subplot(3,4,i);
    plot(damf_true(i,:), 'k', 'LineWidth', 1.5); hold on;
    plot(fd_dkf1_optimized(i,:), 'm--', 'LineWidth', 1);
    plot(fd_dkf2_optimized(i,:), 'b--', 'LineWidth', 1);
    plot(fd_est(i,:), 'g--', 'LineWidth', 1);
    ...
    % Constructing strings with 4 decimal places for precision
    leg1 = 'True';
    leg2 = ['DKF1 (RMSE: ', num2str(rmse_dkf1, '%.4f'), ')'];
    leg3 = ['DKF2 (RMSE: ', num2str(rmse_dkf2, '%.4f'), ')'];
    leg4 = ['EKF  (RMSE: ', num2str(rmse_ekf, '%.4f'), ')'];
    
    legend({leg1, leg2, leg3, leg4}, 'FontSize', 8, 'Location', 'best');
end
...
% nahole (0.6069, 0.2786,0.2932, 0.6899,0.8940,0.1404, 0.1428,0.9633, 0.6902,0.3785, 0.3702,  0.6819)
