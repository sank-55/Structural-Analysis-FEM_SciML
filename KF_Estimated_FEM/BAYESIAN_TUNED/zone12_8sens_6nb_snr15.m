...
%%   predicting very well 15% snr good but need to tune a little bit for dkf2 
clear all; close all; clc;

%% 1. PHYSICAL PARAMETERS & SYSTEM SETUP
p=1000; ro=2700; yo=70*10^9; neu=0.3; h=0.001;
a=0.6; b=0.4; Nm=6; Ndam=12; N=5000; deltat=0.0001; w_1=100*pi;
nObserv=8;
...
for i = 800:N,  damf_true(8,i) = (i-800) * 0.4 / (N-800); end
...
a_t = Mgm \ (Fgf*sin(w_1*0) - Cgc*v_t - Kgk_undamaged*x_t); 
...
for i = 2:N
    ...
    K_curr = Kgk_undamaged;
    for d = 1:Ndam
        K_curr = K_curr - damf_true(d,i) * squeeze(Kd(d,:,:)); 
    end
    
    % 2. Effective Stiffness Matrix
    K_hat = K_curr + a1*Cgc + a0*Mgm;
    
    % 3. Effective Force Vector (Predictor)
    F_ext = Fgf * sin(w_1*t);
    F_hat = F_ext + Mgm*(a0*x_t + a2*v_t + a3*a_t) + Cgc*(a1*x_t + a4*v_t + a5*a_t);
    ...
    zst_measured(:,i) = [Phi_mat *( x_new + x_new*0.15*randn); Phi_mat *( v_new + v_new*0.15*randn)];
    zacc_measured(:,i) = Phi_mat *( a_new + a_new*0.15*randn);
...
optimVars_ekf = [
    %  -18.531        -13.219        -14.798          -10    -1.2706(0.060371)
    % -17.804        -13.566        -14.991        -9.9706      -1.4006(0.059497)
    optimizableVariable('p_state_exp', [-18, -16], 'Type', 'real')
    optimizableVariable('p_param_exp', [-14, -12], 'Type', 'real')
    optimizableVariable('q_state_exp', [-15, -14], 'Type', 'real')
    optimizableVariable('q_param_exp', [-10, -8], 'Type', 'real')
    optimizableVariable('r_exp', [-2, 1], 'Type', 'real')
];
...
optimVars_dkf2 = [
% -21.21         -26.565          13.561         -8.2603          2.432          0.88177       0.89838(0.043242)
%-22.561         -27.479          13.586         -9.3651          3.233          -1.1339       0.89912(0.038808)
    optimizableVariable('p_state_dkf2', [-23, -21], 'Type', 'real')
    optimizableVariable('p_param_dkf2', [-28, -26], 'Type', 'real')
    optimizableVariable('q_state_dkf2', [13, 14], 'Type', 'real')
    optimizableVariable('q_param_dkf2', [-10, -9], 'Type', 'real')
    optimizableVariable('r_state_dkf2', [3, 4], 'Type', 'real')
    optimizableVariable('r_param_dkf2', [-2, 1], 'Type', 'real')
...
optimVars_dkf1 = [
    %-21.666         -24.627          12.608         -8.7053         -0.10242        0.36938   (0.042719)
   % -22.735         -25.155          13.722         -8.9188          1.2583        -0.042633(0.031255) 
    optimizableVariable('p_state_dkf1', [-23, -21], 'Type', 'real')
    optimizableVariable('p_param_dkf1', [-26, -24], 'Type', 'real')
    optimizableVariable('q_state_dkf1', [13, 14], 'Type', 'real')
    optimizableVariable('q_param_dkf1', [-9, -7], 'Type', 'real')
    optimizableVariable('r_state_dkf1', [0, 2], 'Type', 'real')
    optimizableVariable('r_param_dkf1', [-1, 0], 'Type', 'real')
...
leg_labels = {'True', 'DKF1[RMSE:0.043919]', 'DKF2[RMSE:0.043978]', ...
              'EKF[RMSE:0.057763]  \bf[Sensor:8 (SNR:15%)]\rm'};
...
%%
% Assuming your variables are named array1, array2, array3
dataCells = {fd_dkf1_optimized, fd_dkf2_optimized, fd_est};
sheetNames = {'fd_dkf1_optimized', 'fd_dkf2_optimized', 'fd_est'};

filename = 'fd_Data_snr20.xlsx';

for i = 1:3
    % writematrix(data, filename, 'Sheet', sheetName)
    writematrix(dataCells{i}, filename, 'Sheet', sheetNames{i});
end

fprintf('Successfully exported %s with 3 sheets.\n', filename);
%% --- HELPER: EKF FUNCTION ---
function [score, fd_history] = run_ekf_internal(p, M, K0, C, Kd_all, Phi, Fgf, nObs, Nm, Ndam, dt, N, w1, z, truth)
    ...
    P = blkdiag(P_state*eye(2*Nm), P_param*eye(Ndam));
    Q = blkdiag(Q_state*eye(2*Nm), Q_param*eye(Ndam));
    R = R_val * eye(nObs);
